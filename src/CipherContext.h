        ...
        case Mode::PCBC:
            {
                ...
                if (rest != 0) {
                    ...
                    XOR(tmp_text, last_block, output + block_count * block_size, block_size);
        ...
        case Mode::PCBC:
            {
                ...
                XOR(service_block, service_block, iv, block_size);
                ...
                if (rest) {
                    ...
                    XOR(last_block, last_block, data + block_count * block_size, block_size);
                    XOR(last_block, last_block, output + (block_count - 1) * block_size, block_size);
        ...
        case Mode::CFB:
            {
                ...
                XOR(service_block, service_block, data, block_size);
                ...
                if (rest) {
                    ...
                    // *tmp_out ^= *tmp_text;
                    XOR(last_block, last_block, data + (block_count + 1) * block_size, block_size);
        ...
        case Mode::RandomDelta:
            {
                ...
                memcpy(tmp_iv + sizeof(uint64_t),
                    rnd_iv + sizeof(uint64_t), block_size - sizeof(uint64_t));
                *reinterpret_cast<uint64_t*>(tmp_iv) =
                    *reinterpret_cast<uint64_t*>(rnd_iv) + (block_count - 1) * delta;

                algorithm->encrypt(tmp_iv, service_block, key);

                XOR(service_block, service_block, data + block_count * block_size, block_size);
                ...
                if (rest != 0) {
                    ...
                    *reinterpret_cast<uint64_t*>(tmp_iv) =
                        *reinterpret_cast<uint64_t*>(rnd_iv) + block_count * delta;
                    algorithm->encrypt(tmp_iv, last_block, key);
                    XOR(last_block, last_block,
                        data + (block_count + 1) * block_size, block_size);
