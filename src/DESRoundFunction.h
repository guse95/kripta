...
class DESRoundFunction : IRoundFunction
{
    ...
    int S_Table[][][] = {
        ...
        {
            {15, 1, 8, 4, 6, 11, 3, 4, 9, 7, 2, 13, 12, 0, 5, 10},
    ...
    void roundFun(uint8_t* text, uint8_t* result, uint8_t* roundKey) override
    {
        ...
        *tmp_text ^= *tmp_key;
        ...
        for (int i = 0; i < 8; i++)
        {
            auto row_ind = ((*tmp_text & (33 << (6 * i + 16))) >> (6 * i + 16));
            auto col_ind = ((*tmp_text & (30 << (6 * i + 16))) >> (6 * i + 16));
            auto tmp_res = reinterpret_cast<uint32_t*>(result);
